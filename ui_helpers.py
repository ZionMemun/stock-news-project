...
def init_ui_state():
    ...
    for key, value in defaults.items():
        if key not in st.session_state:  # keep existing values
            st.session_state[key] = value


def get_theme_colors():
    """
    Return dashboard theme colors.
    """
    return {
        "bg": "#0b1220",
        "bg_2": "#0f172a",
...
def render_global_sidebar(df=None, include_show_urls=False):
    """
    Render sidebar filters and keep state.
    """
...
def convert_utc_to_timezone(series, timezone_name):
    ...
    dt_series = pd.to_datetime(series, errors="coerce", utc=True)  # safe datetime parse
    return dt_series.dt.tz_convert(timezone_name)


def prepare_dataframe(df, timezone_name, add_display_id=False):
    """
    Prepare dataframe for display and sorting.
    """
    df = df.copy()
    ...
    df = df.reset_index(drop=True)  # reset after sorting

    if add_display_id:
        df.insert(0, "display_id", df.index + 1)  # UI-only id
...
def apply_filters(df):
    """
    Apply selected sidebar filters to dataframe.
    """
    filtered_df = df.copy()
